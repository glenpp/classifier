...
class Classifier:
    def __init__(self, db, text):
        ...
        if str(type(self.db)) == "<class 'MySQLdb.connections.Connection'>":
            ...
            raise TypeError("Don't know dbtype: {}".format(str(type(self.db))))
...
#            my $wordid
    ...
    def classify(self, classifications, useorder=None):    # $useorder = 0.0-1.0 for the factor of order information
        ...
        for clas in classifications:
            ...
            if messages[clas] == 0.0:
                messages[clas] = 1e-12    # use a safe tiny value so the math works
            total += messages[clas]
            if bindclassifications != '':
                bindclassifications += ','
            if self.dbtype == 'MySQL':
...
#            print STDERR "finalscores: $clas => $prob[$clas]\n"
    ...
    def cleanfrequency(self, threshold, limit=None):
        limitstr = ''
        if limit is not None:
            limitstr = ' LIMIT %d' % limit
        dbcur = self.db.cursor()
        if self.dbtype == 'MySQL':
            dbcur.execute('DELETE FROM ClassifierFrequency WHERE Frequency < %s' + limitstr, [threshold])
            dbcur.execute('DELETE FROM ClassifierOrderFrequency WHERE Frequency < %s' + limitstr, [threshold])
        else:
            dbcur.execute('DELETE FROM ClassifierFrequency WHERE Frequency < :threshold' + limitstr, [threshold])
            dbcur.execute('DELETE FROM ClassifierOrderFrequency WHERE Frequency < :threshold' + limitstr, [threshold])
        dbcur.execute('DELETE FROM ClassifierWords WHERE id NOT IN (SELECT Word FROM ClassifierFrequency) AND id NOT IN (SELECT Word FROM ClassifierOrderFrequency) AND id NOT IN (SELECT PrevWord FROM ClassifierOrderFrequency WHERE PrevWord IS NOT NULL)' + limitstr)
        self.db.commit()    # finish transaction to avoid slow synchronous writes
